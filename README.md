...
│   ├── spark_session.py      # Spark session config
│   └── etl/
│       ├── bronze.py         # Bronze layer ETL
│       └── silver.py         # Silver layer + DQ checks
│
├── tests/
│   └── test_silver_dq.py     # Data quality smoke tests
│
├── warehouse/               # Generated by pipeline (gitignored)
│   ├── bronze/
│   └── silver/
│       └── dq_report/
│
